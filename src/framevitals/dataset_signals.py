...
# ---------------------------------------------------------------------------
# Leakage heuristic (signal-level, not full analysis)
# ---------------------------------------------------------------------------

def _detect_potential_leakage(df: pd.DataFrame, column_roles: dict) -> bool:
    """
    Quick heuristic: if any non-ID numeric pair has |correlation| > 0.98
    it may indicate leakage or redundancy.
    """
...
def detect_dataset_signals(df: pd.DataFrame, profile: dict) -> dict:
    ...
    Returns:
        dict with boolean/numeric signals + column lists
    """
    rows, cols = df.shape
    ...
    return {
        ...
        # --- boolean flags (drive the analysis selector) ---
