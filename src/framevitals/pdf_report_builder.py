...
========================================
Generates the FrameVitals dataset report PDF.
...
def _cover_page(c: Composer, result: dict):
    ...
    lay.text(
        MARGIN_L, c.y,
        "Generated by FrameVitals' analytical pipeline.",
        fontsize=10, color=INK_3, fontweight="700",
        ha="left", va="top",
    )
    lay.text(
...
def _render_executive_summary(c: Composer, result: dict):
    ...
    lead = (
        ...
        f"FrameVitals ran in {str(mode).upper()} mode and selected "
...
def _render_cleaning(c: Composer, result: dict):
    ...
    c.section("Cleaning Summary", kind="cleaning",
              subtitle="Summary of applied cleaning actions and health impact.")
...
def _render_ai_report(c: Composer, result: dict):
    ...
    c.section("AI Analyst Report", kind="ai",
              subtitle="Narrative interpretation of the dataset by FrameVitals' analyst model.")
...
def _build_toc_sections(result: dict) -> list[tuple[str, str, str]]:
    ...
    if result.get("cleaning"):
        sections.append((f"{len(sections)+1:02d}", "Cleaning Summary",
                          "Before / after health and applied cleaning actions."))
    ...
    if ai.get("text"):
        sections.append((f"{len(sections)+1:02d}", "AI Analyst Report",
                          "Narrative interpretation by the FrameVitals analyst model."))
...
def generate_pdf_report(
    ...
    report_title: str = "FrameVitals Dataset Report",
) -> Path:
